...
public class HourCalculatorServiceTests
{
    ...
    public void CalculateHour_ShouldReturnCorrectHours_WhenRecurrent(
        string date, string startHour, string endHour, int hourlyInterval, DailyHourFrequency frequency,
        string[] expectedHours)
    {
        // Arrange
        var hourCalculator = new HourCalculatorService();
        var timeRange =
            new HourTimeRange(TimeSpan.Parse(startHour), TimeSpan.Parse(endHour), hourlyInterval, frequency);
    ...
    public void CalculateHour_ShouldThrowException_WhenHourlyIntervalIsNonPositive()
    {
        // Act
        Action act = () =>
            new HourTimeRange(new TimeSpan(9, 0, 0), new TimeSpan(17, 0, 0), 0, DailyHourFrequency.Recurrent);
